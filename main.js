...
function getWeatherData() {
	let p = new Promise(function(resolve, reject) {
		superagent.get(weatherUrl + weatherCity).end(function(err, res) {
			if (err) {
				reject(err);
			}
			let threeDaysData = [];
			...
			resolve({ threeDaysData, weatherTip });
		});
	});
	return p;
}
...
sendImmediately && getAllDataAndSendMail();
